var vuexRecepcao = {
	...
	mutations: {
        ...
        REPLACE_ATENDIMENTO: function(state, novo){
            ...
            state.atendimentosDataSelecionada.forEach(function(atendimento, index){

                if(atendimento.id_atendimento == novo.id_atendimento ){
                    console.log( '========================' )
                    console.log( index )
                    console.log( state.atendimentosDataSelecionada[index] )
                    console.log( novo )
                    console.log( '========================' )
                    // state.atendimentosDataSelecionada[index] =  novo;
                    Vue.set(state.atendimentosDataSelecionada, index, novo)
                }
	...
	actions: {
        ...
        FETCH_ATENDIMENTOS_DATA_SELECIONADA: function({ commit, state, dispatch  }, payload){
            ...
            request.done(function(response){
                ...
                dispatch('_PROCESSAR_ATENDIMENTOS_PESQUISA', response);
            });
        ...
        _PROCESSAR_ATENDIMENTOS_PESQUISA: function({ commit, state, dispatch  },atendimentoEncontrados){
            ...
            // commit('SET_ATENDIMENTOS_DATA_SELECIONADA', atendimentoEncontrados);

            commit('SET_ATENDIMENTOS_DATA_SELECIONADA', atendimentoEncontrados.data.atendimentos);
...
// --------------------------------------------------------------------
	...
	getters: {
        ...
        atendimentosPesquisa: function(state){
console.log('Getter: atendimentosPesquisa');
            var atendimentos = [];

            state.atendimentosDataSelecionada.forEach(function(atendimento){
                atendimentos.push(padronizarObjetoAtendimento(atendimento));
            });
